...
def get_passengers_by_age():
    ...
    rds_insert_time = timeit.timeit(lambda: rds.get_passengers_by_age(27), number=1)
    results.append(['rds', rds_insert_time])

    redshift_insert_time = timeit.timeit(lambda: redshift.get_passengers_by_age(27), number=1)
    results.append(['redshift', redshift_insert_time])

    redis_insert_time = timeit.timeit(lambda: redis.get_passengers_by_age(27), number=1)
    results.append(['redis', redis_insert_time])

    dynamodb_insert_time = timeit.timeit(lambda: dynamodb.get_passengers_by_age(27), number=1)
    results.append(['dynamodb', dynamodb_insert_time])

    df = pd.DataFrame(results, columns=['DB', 'Time'])
    df.to_csv('./select_many_by_age.csv', index=False)

get_passengers_by_age()